...
# drop geometry column too
geometry <- sl19areas$geometry
sl19num <- sl19num %>% st_drop_geometry()

# normalize data to mean 1, std dev 1
sl19scaled <- as.data.frame(scale(sl19num))
...
ggplot(df) +
  geom_histogram(aes(residuals), fill="blue", bins = 20)
```
...
spdep::lm.morantest(basic_linear_model, queen_weights)

# spatial lag model
# df <- df %>% st_drop_geometry()
df <- df %>% select(-c("geometry", "residuals"))
spatial_lag_model <- lagsarlm(hv271 ~ .,
                   data=df,
                   queen_weights, 
                   zero.policy = TRUE)
...
dt <- train(hv271 ~ ., data=df, method="rpart", trControl = cv)
print(dt)
rpart.plot(dt$finalModel, main="Decision Tree with 5-Fold Cross Validation")
...
## Deeper DTree without Cross Validation (manually set cp=0.001) for nice in between
control <- rpart.control(minsplit = 1, minbucket = 1, maxdepth = 10)
cp3 <- 0.001
